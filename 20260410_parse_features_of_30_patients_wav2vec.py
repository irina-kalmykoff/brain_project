# Converted from 20260410_parse_features_of_30_patients_wav2vec.ipynb
...
run_config = {
    ...
    'min_frames': 0,
    'max_frames': 300,
    # Step 5b: choose ONE approach a/b
    'stacking_order': 7,
...
pipeline = Dutch30Pipeline(
    ...
    use_whisperx=run_config['use_whisperx']
)
...
# Attach WhisperX model for  step2
# pipeline.whisperx_model    = model_a
# pipeline.whisperx_metadata = metadata
...
stk_tag = f'stk{so}_s{ss}' if use_stacking else (f'norm{tf}' if use_resampling else 'raw')
FRAME_CACHE = f'cache_frames_P{pr[0]:02d}-P{pr[1]:02d}.pkl'          # pre-5a/b/c
STEP5_CACHE = f'cache_step5_P{pr[0]:02d}-P{pr[1]:02d}_{stk_tag}.pkl' # post-5c
...
# ═══════════════════════════════════════════════════════════════════════════════
# 1.  Try post-step5c cache  (fastest: skip everything)
# ═══════════════════════════════════════════════════════════════════════════════
if os.path.exists(STEP5_CACHE):
    ...
    cached_train        = state['cached_train']   # frame-level, for sweep
    cached_test         = state['cached_test']
    count(pipeline, "Loaded from step-5 cache")

# ═══════════════════════════════════════════════════════════════════════════════
# 2.  Try frame-level cache  (skip steps 1-5_accumulate, redo 5a/b/c only)
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    pipeline.step5a_filter_by_frame_count(
        min_frames=run_config['min_frames'], max_frames=run_config['max_frames'])
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if os.path.exists(STEP3_CHECKPOINT):
        print(f"✓ Step-3 checkpoint found: {STEP3_CHECKPOINT}")
...
def build_mfa_features(pipeline, run_config):
    """
    Replace step4 + step5_accumulate with MFA-aligned phoneme segments.
    
    For each patient:
      1. Load MFA TextGrids (precise phoneme timestamps)
      2. For each phoneme, slice the raw EEG and extract high-gamma features
      3. Build pipeline.train / pipeline.test in the same format as step5_accumulate
      4. Fall back to pipeline's own detection for sentences without TextGrids
    
    Returns: (train_dict, test_dict) ready for step5a/b/c
    """
    config     = pipeline.config
    eeg_sr     = config.eeg_sr        # 1024
    audio_sr   = config.audio_sr      # 48000
    win_len    = config.window_length  # 0.03
    frameshift = config.frameshift    # 0.005
    
    mfa_output = os.path.join(DUTCH_30_PATH, 'mfa_output')
    
    # Min EEG samples needed for extractHG to produce at least 1 frame
    min_samples = int(win_len * eeg_sr) + 1
    
    # Accumulators for train and test
    accum = {
        'train': {
    ...
    # Build a lookup: which word instances are train vs test per patient
    # split_result['train'][pid][word] = [instance_indices]
    # split_result['test'][pid][word]  = [instance_indices]
    ...
    for pid in patients:
        ...
        # Apply channel exclusions if available
        ...
        # Build train/test instance lookup for this patient
        train_instances = {}  # word → set of instance indices
        test_instances  = {}
        if pid in split_result.get('train', {}):
            for word, indices in split_result['train'][pid].items():
                train_instances[word] = set(indices)
        if pid in split_result.get('test', {}):
            for word, indices in split_result['test'][pid].items():
                test_instances[word] = set(indices)
        
        # Track which sentence indices map to which sentence_list entries
        # sentence_list alternates: [sentence, rest, sentence, rest, ...]
        # Sentence index in MFA = i, but sentence_list index = i*2 (even entries)
        sentence_list = word_data['sentence_list']
        
        pid_mfa_count = 0
        pid_fallback_count = 0
        
        # Process each sentence
        for sent_list_idx, sent in enumerate(sentence_list):
            text = sent['text'] if isinstance(sent, dict) else sent
            if not text:  # skip rest intervals
                continue
            
            start_eeg_idx = sent['stim_start_idx']
            end_eeg_idx   = sent['stim_end_idx']
            sentence_eeg  = raw_eeg[start_eeg_idx:end_eeg_idx]
            
            # MFA sentence index = position among non-empty sentences
            # The export used enumerate over sentence_list, keeping original indices
            # sent000 = sentence_list[0], sent002 = sentence_list[2], etc.
            mfa_sent_idx = sent_list_idx  # matches the export filename index
            
            if mfa_sent_idx in alignments and len(alignments[mfa_sent_idx]) > 0:
                # ── Use MFA alignment ────────────────────────────────────
                phones = alignments[mfa_sent_idx]
                
                for ph in phones:
                    phone_label = ph['phone']
                    word_text   = ph['word'].lower() if ph['word'] else '?'
                    
                    # Convert MFA timestamps to EEG sample indices (relative to sentence)
                    ...
                    eeg_segment = sentence_eeg[ph_start:ph_end]
                    
                    if eeg_segment.shape[0] < min_samples:
                        continue  # too short for feature extraction
                    
                    # Extract high-gamma features
                    try:
                        feat = extractHG(eeg_segment, eeg_sr,
                    ...
                    # Match word to split_result
                    split = None
                    if word_text in train_instances:
                        split = 'train'
                    elif word_text in test_instances:
                        split = 'test'
                    else:
                        # Try case-insensitive match
                        for w in train_instances:
                            if w.lower() == word_text:
                                split = 'train'
                                break
                        if split is None:
                            for w in test_instances:
                                if w.lower() == word_text:
                                    split = 'test'
                                    break
                    ...
                    accum[split]['phoneme_instance_ids'].append(global_instance_id)
                    accum[split]['phoneme_durations_samples'].append(eeg_segment.shape[0])
                    
                    global_instance_id += 1
                    pid_mfa_count += 1
            
            # else: sentence not in MFA → skip for now
            # (could add fallback to wav2vec here if desired)
        
        print(f"  {pid}: MFA={pid_mfa_count} phonemes"
              f"  ({len(alignments)}/{len([s for s in sentence_list if (s['text'] if isinstance(s, dict) else s)])} sentences)")
    
    # Summary
    for split in ['train', 'test']:
        n = len(accum[split]['features'])
        n_ph = len(set(accum[split]['phoneme_labels']))
        print(f"  {split}: {n} phoneme samples, {n_ph} unique phonemes")
...
# Make sure pipeline has split_result and patient_data loaded
# (from step3 checkpoint or from running steps 1-3)
...
# Cache frame-level state (before step5a/b/c)
cached_train = copy.deepcopy(pipeline.train)
cached_test  = copy.deepcopy(pipeline.test)

count(pipeline, "After MFA accumulation")

# ── Continue with step 5a/b/c as before ──────────────────────────────────────
so = run_config['stacking_order']
ss = run_config['stacking_step_size']
tf = run_config['target_frames']
use_stacking   = so is not None
...
def load_mfa_alignments(pid, mfa_output_dir=None):
    ...
    Returns dict: sentence_idx → list of (phone, start_s, end_s)
    """
    ...
    for tg_file in sorted(os.listdir(pid_dir)):
        if not tg_file.endswith('.TextGrid'):
            continue
        # Extract sentence index from filename: P21_sent000.TextGrid → 0
        try:
        ...
        # Collect phones with their parent word
        phones = []
...
def mfa_phones_to_eeg_segments(phones, sentence_eeg, sentence_start_eeg_idx,
                                eeg_sr=1024, audio_sr_mfa=16000):
    """
    Convert MFA phone timestamps to EEG slices.
    
    Args:
        phones: list of {'phone', 'start_s', 'end_s', 'word'} from load_mfa_alignments
        sentence_eeg: EEG array for this sentence (n_samples, n_channels)
        sentence_start_eeg_idx: stim_start_idx from sentence_list (EEG sample index)
        eeg_sr: EEG sampling rate
    
    Returns:
        list of {'phone', 'word', 'eeg_segment', 'start_eeg', 'end_eeg'}
    """
    segments = []
    n_eeg_samples = sentence_eeg.shape[0] if sentence_eeg.ndim > 1 else len(sentence_eeg)
    
    for ph in phones:
        # MFA timestamps are relative to the wav file start (= sentence start)
        start_eeg = int(ph['start_s'] * eeg_sr)
        end_eeg   = int(ph['end_s']   * eeg_sr)
        
        # Bounds check
        start_eeg = max(0, min(start_eeg, n_eeg_samples - 1))
        end_eeg   = max(start_eeg + 1, min(end_eeg, n_eeg_samples))
        
        eeg_slice = sentence_eeg[start_eeg:end_eeg]
        
        segments.append({
            'phone':       ph['phone'],
            'word':        ph['word'],
            'eeg_segment': eeg_slice,
            'start_eeg':   start_eeg,
            'end_eeg':     end_eeg,
            'duration_ms': (ph['end_s'] - ph['start_s']) * 1000,
        })
    
    return segments


# ── Test it on P21 sentence 0 ────────────────────────────────────────────────
pid = 'P21'
alignments = load_mfa_alignments(pid)
print(f"{pid}: loaded MFA alignments for {len(alignments)} sentences")

# Get sentence EEG
word_data = pipeline.split_result['word_segments_dict'][pid]
sent = word_data['sentence_list'][0]  # sentence 0
text = sent['text'] if isinstance(sent, dict) else sent
...
sentence_eeg = raw_eeg[start_idx:end_idx]

if 0 in alignments:
    segments = mfa_phones_to_eeg_segments(alignments[0], sentence_eeg, start_idx)
    print(f"\nSentence 0: '{text}'")
    print(f"EEG shape: {sentence_eeg.shape}, duration: {sentence_eeg.shape[0]/1024*1000:.0f}ms")
    print(f"\n{'Phone':<8} {'Word':<15} {'Duration':>10} {'EEG samples':>12}")
    print('-' * 50)
    for s in segments:
        print(f"{s['phone']:<8} {s['word']:<15} {s['duration_ms']:>8.0f}ms "
              f"{s['eeg_segment'].shape[0]:>10}")
else:
    print("Sentence 0 not in MFA alignments")
...
# ── DIAGNOSTIC: Where are phonemes lost? ─────────────────────────────────
import os
import tgt
import numpy as np
from config import DUTCH_30_PATH
...
def diagnose_mfa_loss(pipeline, run_config):
    """Show exactly where phonemes are lost between raw TextGrids and build_mfa_features."""
    config     = pipeline.config
    eeg_sr     = config.eeg_sr        # 1024
    audio_sr   = config.audio_sr      # 48000
    win_len    = config.window_length  # 0.03
    min_samples = int(win_len * eeg_sr) + 1  # 31
    ...
    print(f"{'PID':<6} {'TextGrid':>10} {'No TG':>8} {'TooShort':>10} {'Kept':>8} {'Loss%':>7}")
    print("-" * 55)
    
    total_tg = total_notg = total_short = total_kept = 0
    ...
    for pid in patients:
        ...
        # Apply same channel exclusions as build_mfa_features
        if hasattr(pipeline, 'patient_data') and pid in pipeline.patient_data:
            pdata = pipeline.patient_data[pid]
            if 'channel_mask' in pdata:
                raw_eeg = raw_eeg[:, pdata['channel_mask']]
        ...
        pid_tg_phones = 0      # phones in TextGrids for matched sentences
        pid_no_tg_phones = 0   # phones in sentences without TextGrids
        pid_too_short = 0      # phones skipped by min_samples
        pid_kept = 0           # phones that survive
        ...
        for sent_list_idx, sent in enumerate(sentence_list):
            ...
            sent_len  = end_eeg - start_eeg
            
            if sent_list_idx in alignments and len(alignments[sent_list_idx]) > 0:
                phones = alignments[sent_list_idx]
                pid_tg_phones += len(phones)
                ...
                # Count how many phones we'd expect (estimate from other sentences)
                # For now just note the sentence was missed
                pid_no_tg_phones += 1  # count sentences, not phones
        
        # Also count raw TextGrid phones for comparison
        raw_count = sum(len(phones) for phones in alignments.values())
        n_sent_with_text = sum(1 for s in sentence_list 
                               if (s['text'] if isinstance(s, dict) else s))
        n_sent_with_tg = len(alignments)
        
        loss_pct = (1 - pid_kept / raw_count * 100 / 100) * 100 if raw_count > 0 else 0
        
        print(f"{pid:<6} {raw_count:>10} {n_sent_with_text - n_sent_with_tg:>8} sent  "
    ...
    print(f"Phonemes lost to missing TextGrids: {total_tg - total_short - total_kept} "
          f"(estimated)")
...
def analyze_consecutive_predictions(pipeline, min_run=2, n_example_words=12):
    """
    Per patient:
      TRAIN block — how many consecutive phonemes per word were detected,
                    and at which positions within the word.
      TEST block  — prediction accuracy per position within word,
                    top consecutive-correct runs with IPA/*(pred) rendering,
                    example words showing correct vs wrong inline.
    """
    pr   = run_config['patient_range']
    pids = sorted(set(
    ...
    for pid in pids:
        ...
        X_tr     = np.array([np.array(pipeline.train['features'][i]).flatten() for i in tr_idx])
        y_tr_r   = [pipeline.train['phoneme_labels'][i] for i in tr_idx]
        words_tr = [pipeline.train['phoneme_words'][i]  for i in tr_idx]

        te_idx   = [i for i, p in enumerate(pipeline.test['phoneme_participant_ids']) if p == pid]
        X_te     = np.array([np.array(pipeline.test['features'][i]).flatten() for i in te_idx])
        y_te_r   = [pipeline.test['phoneme_labels'][i]  for i in te_idx]
        words_te = [pipeline.test['phoneme_words'][i]   for i in te_idx]

        if len(X_tr) == 0 or len(X_te) == 0:
            print(f"{pid}: no data, skipping"); continue

        # ═══════════════════════════════════════════════════════════════════
        # TRAIN: detection analysis — how many phonemes per word were found
        # ═══════════════════════════════════════════════════════════════════
        print(f"\n{'='*70}")
        print(f"{pid}  train={len(y_tr_r)}  test={len(y_te_r)}")

        # group consecutive same-word runs → position within word
        def word_groups_from(word_list):
            ...
            for word, grp in groupby(samples, key=lambda s: s[0]):
                grp       = list(grp)
                detected  = len(grp)
                expected_ph = pipeline.phonetic_dict.extract_phonemes(word)
                expected  = len(expected_ph) if expected_ph else detected
                groups.append({'word': word, 'detected': detected, 'expected': expected})
            return groups

        train_groups = word_groups_from(words_tr)

        # ── detection by word length ──────────────────────────────────────
        ...
        print(f"\n  TRAIN — detection rate by word length (expected #phonemes):")
        ...
        # ── detection by position within word ─────────────────────────────
        tr_pos = defaultdict(lambda: {'detected': 0, 'expected': 0, 'word_lens': []})
        for wg in train_groups:
            for pos in range(wg['expected']):
                tr_pos[pos]['expected']   += 1
                tr_pos[pos]['word_lens'].append(wg['expected'])
                if pos < wg['detected']:
                    tr_pos[pos]['detected'] += 1

        print(f"\n  TRAIN — detection rate by position within word:")
        print(f"  {'Pos':<6} {'AvgWrdLen':>10} {'Detected':>10} {'Expected':>10} {'Det%':>8}")
        for pos in sorted(tr_pos):
            s    = tr_pos[pos]
            pct  = s['detected'] / s['expected'] * 100 if s['expected'] else 0
            avgl = np.mean(s['word_lens'])
            bar  = '█' * int(pct / 5)
            print(f"  {pos:<6} {avgl:>10.1f} {s['detected']:>10} {s['expected']:>10} "
                  f"{pct:>7.1f}%  {bar}")

        # ═══════════════════════════════════════════════════════════════════
        # TEST: train classifier → per-phoneme correct/wrong
        # ═══════════════════════════════════════════════════════════════════
        ...
        le      = LabelEncoder()
        y_tr_e  = le.fit_transform(y_tr_r)
        known   = [l in le.classes_ for l in y_te_r]

        X_te_k  = X_te_s[[i for i, m in enumerate(known) if m]]
        y_te_k  = le.transform([l for l, m in zip(y_te_r, known) if m])
        words_k = [w for w, m in zip(words_te, known) if m]

        if len(X_te_k) == 0:
            print(f"  No test samples with known labels"); continue
        ...
        # build per-sample records with word-level grouping
        test_samples = [
        ...
        # group consecutive same-word → position within word
        te_word_groups = []
        for word, grp in groupby(test_samples, key=lambda s: s['word']):
            grp      = list(grp)
            exp_ph   = pipeline.phonetic_dict.extract_phonemes(word)
            exp_len  = len(exp_ph) if exp_ph else len(grp)
            te_word_groups.append((word, grp, exp_len))

        # position accuracy stats
        te_pos = defaultdict(lambda: {'correct': 0, 'total': 0, 'word_lens': []})
        for word, phonemes, exp_len in te_word_groups:
            for pos, s in enumerate(phonemes):
                te_pos[pos]['total']   += 1
                te_pos[pos]['correct'] += int(s['correct'])
                te_pos[pos]['word_lens'].append(exp_len)
        ...
        for r in runs: run_dist[r['len']] += 1

        all_results[pid] = dict(accuracy=acc, n_test=len(correct),
                                n_runs=len(runs), longest=longest)

        print(f"\n  TEST — acc={acc:.1%}  samples={len(correct)}  "
              f"runs>={min_run}: {len(runs)}  longest: {longest}")
        print(f"  Run-length distribution: { {k: run_dist[k] for k in sorted(run_dist)} }")

        # render helper: symbol=correct, *(pred)=wrong
        def render_word(word, phonemes):
            parts = [s['true'] if s['correct'] else f"*({s['pred']})"
                     for s in phonemes]
            return f"{word}[{'-'.join(parts)}]"

        # top runs
        print(f"\n  TEST — top runs  (IPA=correct  *(x)=predicted as x):")
        for r in sorted(runs, key=lambda r: r['len'], reverse=True)[:5]:
            run_words = [render_word(w, list(g))
                         for w, g in groupby(r['samples'], key=lambda s: s['word'])]
            print(f"  run={r['len']:2d}  " + '  '.join(run_words))

        # example words from test set
        print(f"\n  TEST — example words from test set:")
        shown, line = 0, "  "
        for word, phonemes, _ in te_word_groups:
            if shown >= n_example_words: break
            line  += render_word(word, phonemes) + "  "
            shown += 1
            if shown % 5 == 0: print(line); line = "  "
        if line.strip(): print(line)

        # position accuracy table with word-length column
        print(f"\n  TEST — prediction accuracy by position within word:")
        print(f"  {'Pos':<6} {'AvgWrdLen':>10} {'Correct':>9} {'Total':>9} {'Acc%':>8}")
        for pos in sorted(te_pos):
            s    = te_pos[pos]
            a    = s['correct'] / s['total'] if s['total'] else 0
            avgl = np.mean(s['word_lens'])
            bar  = '█' * int(a * 20)
            print(f"  {pos:<6} {avgl:>10.1f} {s['correct']:>9} {s['total']:>9} "
                  f"{a:>7.1%}  {bar}")

    return all_results


consec = analyze_consecutive_predictions(pipeline, min_run=2, n_example_words=12)

# try montreal-forced-aligner MFA position alignment
...
def clean_text_for_mfa(text):
    """Strip punctuation and quotes that MFA can't handle."""
    text = text.lower().strip()
    # Remove all quote variants and punctuation attached to words
    text = re.sub(r'["""\'\u201c\u201d\u2018\u2019.,!?;:()\[\]{}<>]', ' ', text)
    # Remove standalone digits (or replace: {'1':'één','2':'twee', etc.})
...
def export_sentences_for_mfa(pid, pipeline, out_dir, eeg_sr=1024, audio_sr=48000):
    ...
    exported  = 0
    skipped   = 0   # real problems
    rest_skip = 0   # expected empty rest intervals
    ...
    for i, sent in enumerate(word_data['sentence_list']):
        ...
        # ── skip rest intervals (empty text) ─────────────────────────────
        ...
        with open(os.path.join(pid_dir, f'{pid}_sent{i:03d}.lab'), 'w', encoding='utf-8') as f:
            f.write(cleaned_text)
        exported += 1

    print(f'{pid}: exported={exported}  rest_intervals={rest_skip}  bad_audio={skipped}')

# Re-export all patients
...
# Download dictionary (try dutch_cv first, fall back to dutch_mfa)
# mfa model download dictionary dutch_cv
# Verify
# mfa model list acoustic
# mfa model list dictionary
# If needed: clean previous output first, then re-align
# mfa align C:\mozg\code\SingleWordProductionDutch\Dutch_30patients\mfa_input dutch_cv dutch_cv C:\mozg\code\SingleWordProductionDutch\Dutch_30patients\mfa_output --clean



#phonemizer for transcription
import os
os.environ['PATH'] += r';C:\Program Files\eSpeak NG'
os.environ['PHONEMIZER_ESPEAK_LIBRARY'] = r'C:\Program Files\eSpeak NG\libespeak-ng.dll'

from phonemizer import phonemize
test = phonemize(['schaatsen', 'vegetarisch', 'kleurpotloden'],
                 language='nl', backend='espeak',
...
# Generate IPA for all OOV words
oov_words_list = sorted(oov_by_word.keys())
print(f"Generating pronunciations for {len(oov_words_list)} OOV words...")

ipa_list = phonemize(oov_words_list, language='nl', backend='espeak',
                     with_stress=False, strip=True)

# Preview a few
for word, ipa in zip(oov_words_list[:10], ipa_list[:10]):
    print(f"  {word:<30} → {ipa}")

# Write in MFA dictionary format: word<tab>phone1 phone2 phone3
custom_dict_path = os.path.join(DUTCH_30_PATH, 'mfa_input', 'oov_dict.txt')
written = 0
skipped = 0
with open(custom_dict_path, 'w', encoding='utf-8') as f:
    for word, ipa in zip(oov_words_list, ipa_list):
        ipa = ipa.strip()
        if not ipa:
            skipped += 1
            continue
        # MFA expects space-separated phones — split each IPA character
        phones = ' '.join(list(ipa))
        f.write(f"{word}\t{phones}\n")
        written += 1

print(f"\nWritten {written} pronunciations, skipped {skipped}")
print(f"Saved to: {custom_dict_path}")


# # In notebook — delete all empty .lab files from mfa_input
# import os
# from config import DUTCH_30_PATH

# mfa_input = os.path.join(DUTCH_30_PATH, 'mfa_input')
# deleted = 0
# for pid_dir in os.listdir(mfa_input):
#     pid_path = os.path.join(mfa_input, pid_dir)
...
#             if not content:
#                 # Remove the matching .wav too
#                 wav_path = fpath.replace('.lab', '.wav')
#                 os.remove(fpath)
#                 if os.path.exists(wav_path):
#                     os.remove(wav_path)
#                 deleted += 1

# print(f"Deleted {deleted} empty lab+wav pairs")
...
# Sanity check — read one TextGrid
tg_files = [f for f in os.listdir(os.path.join(mfa_output, 'P21'))
            if f.endswith('.TextGrid')]
print(f"P21 TextGrids: {len(tg_files)}")

# Read first non-empty one
for tg_file in sorted(tg_files)[:5]:
    tg_path = os.path.join(mfa_output, 'P21', tg_file)
    tg   = tgt.io.read_textgrid(tg_path)
    tier = tg.get_tier_by_name('phones')
    phones = [a for a in tier.annotations if a.text not in ('', 'sp', 'sil', 'spn')]
    if phones:
        print(f"\n{tg_file}:")
        for a in phones:
            print(f"  {a.text:<6} {a.start_time:.3f}–{a.end_time:.3f}s  "
                  f"({(a.end_time-a.start_time)*1000:.0f}ms)")
        break

import os

# Find the MFA dictionary file
mfa_dict_path = os.path.join(
    os.path.expanduser('~'), 'Documents', 'MFA', 
    'pretrained_models', 'dictionary', 'dutch_cv.dict'
)

# If not there, search for it
if not os.path.exists(mfa_dict_path):
    import glob
    candidates = glob.glob(os.path.join(
        os.path.expanduser('~'), '**', 'dutch_cv*'), recursive=True)
    print("Found dictionary files:", candidates)
else:
    print(f"Found: {mfa_dict_path}")

# Load dictionary words
with open(mfa_dict_path, 'r', encoding='utf-8') as f:
    dict_words = set(line.split()[0].lower() for line in f if line.strip())

print(f"Dictionary has {len(dict_words)} words")

# Collect all words from your transcripts
from config import DUTCH_30_PATH
mfa_input = os.path.join(DUTCH_30_PATH, 'mfa_input')

all_words   = set()
oov_words   = set()
oov_by_word = {}   # word → count
...
for pid_dir in os.listdir(mfa_input):
    ...
    for fname in os.listdir(pid_path):
        if not fname.endswith('.lab'):
            continue
        with open(os.path.join(pid_path, fname), encoding='utf-8') as f:
            words = f.read().strip().split()
        for w in words:
            all_words.add(w)
            if w not in dict_words:
                oov_words.add(w)
                oov_by_word[w] = oov_by_word.get(w, 0) + 1

print(f"\nTotal unique words in transcripts: {len(all_words)}")
print(f"OOV words ({len(oov_words)}):")
for w, count in sorted(oov_by_word.items(), key=lambda x: -x[1]):
    print(f"  {w:<30} appears {count:>4}x")
...
x_ms     = [o * FRAMESHIFT_MS for o in offsets]
patients = sorted({pid for res in offset_results.values() for pid in res})

cols = 5
rows = -(-len(patients) // cols)   # ceiling division
fig, axes = plt.subplots(rows, cols, figsize=(cols*4, rows*3.5), sharey=False)
axes = axes.flatten()

for ax, pid in zip(axes, patients):
    accs = [offset_results[o].get(pid, np.nan) for o in offsets]
    ax.plot(x_ms, accs, marker='o', color='steelblue', lw=1.5)
    valid = [(x, a) for x, a in zip(x_ms, accs) if not np.isnan(a)]
    if valid:
        best_x, best_a = max(valid, key=lambda t: t[1])
        ax.axvline(best_x, color='red', ls='--', lw=1.2)
        ax.set_title(f"{pid}  best={best_x:+d}ms\nacc={best_a:.3f}", fontsize=9)
...
plt.suptitle('Accuracy vs temporal offset\n'
             'negative=look earlier (pre-motor), positive=look later (post-onset)',
             y=1.01)
plt.tight_layout()
plt.show()

print(f"\n{'Patient':>10}  {'Best offset':>12}  {'Accuracy':>9}")
for pid in patients:
    accs = [offset_results[o].get(pid, np.nan) for o in offsets]
    if all(np.isnan(a) for a in accs):
        continue
    best_idx = int(np.nanargmax(accs))
    print(f"{pid:>10}  {offsets[best_idx]*FRAMESHIFT_MS:>+11d}ms  {accs[best_idx]:>9.3f}")
...
def run_experiment(pipeline, order=3, class_weight='balanced', use_groups=False,
                   classifier_type='random_forest', use_viterbi=False, random_state=37,
                   stacking_order=None, stacking_step_size=None, scaler_type='standard', 
                   subtract_baseline=False, max_frames=None, min_frames=None, 
                   target_frames=None, min_class_samples=5):
    ...
    if min_frames:          name += f"_min{min_frames}"
    if scaler_type != 'standard': name += f"_{scaler_type}"
    if subtract_baseline:   name += "_bsub"

    params = {
        'order': order, 'class_weight': str(class_weight), 'use_groups': use_groups,
        'classifier_type': classifier_type, 'use_viterbi': use_viterbi,
        'stacking_order': stacking_order, 'stacking_step_size': stacking_step_size,
        'max_frames': max_frames, 'target_frames': target_frames,
        'random_state': random_state, 'scaler_type': scaler_type,
        'subtract_baseline': subtract_baseline, 'min_class_samples': min_class_samples,
    }
    print(f"\nRunning: {name}")
    ...
    for pid in sorted(set(pipeline.train['phoneme_participant_ids'])):
        ...
        valid_classes = {cls for cls, cnt in Counter(train_labels).items() if cnt >= min_class_samples}
        train_feat   = [f for f, l in zip(train_feat,  train_labels) if l in valid_classes]
        train_labels = [l for l in train_labels if l in valid_classes]
        test_feat    = [f for f, l in zip(test_feat,   test_labels)  if l in valid_classes]
        test_labels  = [l for l in test_labels  if l in valid_classes]
        ...
        preds_no_vit, _ = model.predict(test_feat, use_viterbi=False)
        preds_no_vit = [str(p) for p in preds_no_vit]
        preds = [str(p) for p in (model.predict(test_feat, use_viterbi=True)[0] if use_viterbi else preds_no_vit)]

        accuracy = sum(p == t for p, t in zip(preds, test_labels)) / len(test_labels)
        results[pid] = {'accuracy': accuracy, 'n_test': len(test_labels), 'n_train': len(train_feat)}
        print(f"  {pid}: {accuracy:.3f}  (train={len(train_feat)}, test={len(test_labels)})")
...
logger = ExperimentLogger('experiments_v12_new_border_detection.json')

RESULTS_FILE = 'experiments_v12_new_border_detection.json'
done_keys = set()
...
stacking_params = [(5,1),(5,2),(7,1),(7,2),(9,1),(9,2)]
...
for (so, ss), mn, mx in product(stacking_params, min_frames_options, max_frames_options):
    configs.append({'stacking_order': so, 'stacking_step_size': ss,
...
for tf, mn, mx in product(resampling_target_frames, min_frames_options, max_frames_options):
    configs.append({'stacking_order': None, 'stacking_step_size': None,
...
for config, scaler_type, subtract_bl in all_experiments:
    key = (config['stacking_order'], config['stacking_step_size'],
           config['target_frames'], config['min_frames'], config['max_frames'],
           scaler_type, subtract_bl)
...
if 'cached_train' not in dir() or cached_train is None:
    print("WARNING: cached_train not set — cannot run sweep.")
else:
    for i, (config, scaler_type, subtract_bl) in enumerate(experiments, 1):
        ...
        try:
            # ── build per-iteration config by merging with global run_config ──
            iter_config = {**run_config,
                           'stacking_order':    config['stacking_order'],
                           'stacking_step_size':config['stacking_step_size'],
            ...
            # ── restore clean frame-level state ──────────────────────────────
            ...
            pipeline.step5a_filter_by_frame_count(
                min_frames=iter_config['min_frames'],
                max_frames=iter_config['max_frames'],
            )

            if iter_config['stacking_order'] is not None:
                pipeline.step5b_stack_features(
                    model_order=iter_config['stacking_order'],
                    step_size=iter_config['stacking_step_size'],
                )
            elif iter_config['target_frames'] is not None:
                pipeline.step5b_normalize_feature_lengths(
                    target_frames=iter_config['target_frames'],
                )

            pipeline.step5c_collapse_to_phoneme_level()
            pipeline.dutch30_step6_resolve_unknowns()
            pipeline.step7_filter_unknowns(unknown_keep_ratio=iter_config['unknown_keep_ratio'])

            # ── run experiment ────────────────────────────────────────────────
            name, params, results = run_from_config(pipeline, iter_config)
...
with open('experiments_v12_new_border_detection.json', 'r') as f:
