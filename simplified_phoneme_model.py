...
class SimplifiedPhonemeModel(DebugMixin):
    ...
    def evaluate(self, prepared_data):
        ...
        labels_in_test = sorted(set(true_groups) | set(predicted_groups))
        
        # Generate classification report
        class_report = classification_report(
            true_groups, predicted_groups,
            labels = labels_in_test,
            target_names= [all_classes[i] for i in labels_in_test], #all_classes,  # Use the actual classes from the encoder
            output_dict=True, 
            zero_division=0
        )
    ...
    def train_with_grouped_data(self, train_features, train_group_labels, train_participant_ids=None,
        ...
        # Check what groups are actually in your data
        unique_train_groups = set(train_group_labels)
        unique_test_groups = set(test_group_labels)
        all_defined_groups = set(self.phonetic_dict.phoneme_to_group.values())

        print(f"Groups in training: {unique_train_groups}")
        print(f"Groups in test: {unique_test_groups}")
        print(f"Groups defined in dictionary: {all_defined_groups}")
        print(f"Missing in test: {all_defined_groups - unique_test_groups}")
    ...
    def map_phonemes_to_groups(self, phonemes):
        """
        Map individual phonemes to their phoneme groups.
        ...
        for phoneme in phonemes:
            ...
            if phoneme in self.phoneme_to_group:
                group = self.phoneme_to_group[phoneme]
            else:
                # If not directly mapped, search in the phoneme groups
                group = 'unknown'
                for group_name, group_phonemes in self.phoneme_groups.items():
                    if phoneme in group_phonemes:
                        group = group_name
                        break
            groups.append(group)
        return groups    
