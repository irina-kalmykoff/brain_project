# Converted from parse_features_of_30_patients_wav2vec_channel_inspector.ipynb
...
class ChannelInspector:
    ...
    def compute_channel_stats(self, eeg):
        ...
        stats['spike_count'] = np.zeros(n_channels)
        for ch in range(n_channels):
            ...
            stats['spike_count'][ch] = np.sum(np.abs(data) > threshold)
    ...
    def plot_channel_overview(self, patient_id, figsize=(20, 16)):
        ...
        high_spikes = stats['spike_count'] > 20
        suspect_channels = high_std | low_std | high_kurtosis | high_spikes        
        ...
        # Plot 3: Spike counts
        ax = axes[1, 0]
        colors = ['red' if high_spikes[i] else 'steelblue' for i in range(n_channels)]
        ax.bar(range(n_channels), stats['spike_count'], color=colors, alpha=0.7, width=1.0)
        ax.axhline(20, color='red', linestyle='--', label='Threshold: 20')
        ax.set_xlabel('Channel')
        ax.set_ylabel('Spike count (>5 std)')
        ax.set_title(f'{patient_id}: Epileptic spike count (red = high)')
        ax.legend()
        ...
        text += f"  High spikes (epileptic): {sum(high_spikes)}\n"
    ...
    def plot_channel_traces(self, patient_id, channels=None, duration_sec=5, 
        ...
        for i, ch in enumerate(channels):
            ...
            if ch_std > std_median + 5 * std_mad:
                ...
                color = 'purple'
                label_suffix = ' [SPIKY]'
            elif stats['spike_count'][ch] > 20:
                color = 'magenta'
                label_suffix = ' [EPILEPTIC]'
            else:
                color = 'black'
                label_suffix = ''
    ...
    def auto_detect_bad_channels(self, patient_id, 
                                  spike_threshold=20,
                                  std_factor=5,
                                  kurtosis_threshold=10,
                                  flat_factor=0.1,
                                  duration_sec=30):
        """Automatically detect bad channels including epileptic ones.

        Runs spike detection on a longer segment for reliability.

        Args:
            patient_id: str, patient ID.
            spike_threshold: int, max spikes (>5 std) allowed.
            std_factor: float, std outlier multiplier.
            kurtosis_threshold: float, max excess kurtosis.
            flat_factor: float, fraction of median std below
                which a channel is considered flat.
            duration_sec: float, seconds to analyze.

        Returns:
            dict with exclude_indices, keep_indices, and per-channel
            rejection reasons.
        """
        eeg = self.get_patient_eeg(patient_id, 
        ...
        for ch in range(n_channels):
            ...
            if stats['std'][ch] > std_median + std_factor * std_mad:
                ch_reasons.append('high_std')

            if stats['std'][ch] < std_median * flat_factor:
                ch_reasons.append('flat')

            if stats['kurtosis'][ch] > kurtosis_threshold:
                ch_reasons.append('high_kurtosis')

            if stats['spike_count'][ch] > spike_threshold:
                ch_reasons.append('epileptic_spikes')

            if ch_reasons:
                reasons[ch] = ch_reasons
                exclude.append(ch)

        keep = [ch for ch in range(n_channels) if ch not in exclude]
        ...
        print(f"\n{patient_id}: {len(exclude)} bad channels, "
              f"{len(keep)} clean")
        for ch, ch_reasons in sorted(reasons.items()):
            print(f"  ch {ch}: {', '.join(ch_reasons)}")

        # Store in exclusions dict
        self.exclusions[patient_id] = result

        return result

    def auto_detect_all_patients(self, patient_ids=None, **kwargs):
        """Run auto-detection on multiple patients.

        Args:
            patient_ids: list of str. If None, uses all patients
                in word_segments_dict.
            **kwargs: passed to auto_detect_bad_channels.

        Returns:
            dict mapping patient_id to detection results.
        """
        if patient_ids is None:
            patient_ids = sorted(self.word_segments_dict.keys())

        results = {}
        for pid in patient_ids:
            try:
                results[pid] = self.auto_detect_bad_channels(pid, **kwargs)
            except Exception as e:
                print(f"  {pid}: FAILED - {e}")

        # Summary
        print(f"\n{'=' * 50}")
        print(f"{'Patient':<10} {'Total':>6} {'Excluded':>9} {'Kept':>6}")
        print("-" * 50)
        for pid, r in sorted(results.items()):
            print(f"{pid:<10} {r['n_channels']:>6} "
                  f"{r['n_excluded']:>9} {r['n_kept']:>6}")

        return results

    def get_clean_channel_mask(self, patient_id):
        """Get boolean mask for clean channels.

        Args:
            patient_id: str, patient ID.

        Returns:
            np.array of bool, True for clean channels.
        """
        if patient_id not in self.exclusions:
            raise ValueError(
                f"No exclusions for {patient_id}. "
                f"Run auto_detect_bad_channels first."
            )

        exc = self.exclusions[patient_id]
        mask = np.ones(exc['n_channels'], dtype=bool)
        for ch in exc['exclude_indices']:
            mask[ch] = False
        return mask
...
# # Auto-detect for one patient
# result = inspector.auto_detect_bad_channels("P21")

# # Visualize to verify
# inspector.plot_channel_overview("P21")
# inspector.plot_detailed_inspection("P21", channels=result['exclude_indices'])

# Run for all sentence patients
inspector.auto_detect_all_patients(
...
# mask = inspector.get_clean_channel_mask("P23")
# # Apply: features = features[:, mask]
...
inspector.plot_all_channels_grid('P30')
