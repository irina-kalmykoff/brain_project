# What "maximally flat" means for the Butterworth filter,
# compared with the Hilbert path's implicit boxcar smoother
# and with other classical IIR filters.
#
# Five panels:
#   CELL 2  — Magnitude response: Butterworth vs 50 ms boxcar (the actual swap)
#   CELL 3  — Passband zoom: what "maximally flat" looks like (linear scale)
#   CELL 4  — Butterworth vs Chebyshev I, Chebyshev II, Elliptic
#   CELL 5  — Impulse response: time-domain shape
...
# ═══════════════════════════════════════════════════════════════════════════════
# This is the head-to-head plot for the change in your pipeline.
...
ax.set_title('Magnitude response (dB) — the smoking gun')
...
# ═══════════════════════════════════════════════════════════════════════════════
# Summary of what each panel shows
# ═══════════════════════════════════════════════════════════════════════════════
# CELL 2:  the actual swap — Butterworth has a clean monotonic rolloff;
#          the 50 ms boxcar's first sidelobe at ~30 Hz leaks −13 dB.
#
# CELL 3:  "maximally flat" defined visually — Butterworth hugs 1.0 across
#          the passband, then bends down.  Higher order = flatter passband
#          and sharper rolloff (no free lunch — it costs more compute
#          and adds more group delay).
#
# CELL 4:  the four classical IIR families:
#            Butterworth — flat | flat | gentle    ← we picked this
#            Cheby I     — RIPPLE | flat | sharper
#            Cheby II    — flat | RIPPLE | sharper
#            Elliptic    — RIPPLE | RIPPLE | sharpest
#          Phoneme content sits at the passband edge — passband ripples
#          would modulate the very signal we want to extract, so we avoid
#          Cheby I and Elliptic.  Cheby II is fine but offers little
#          benefit at order 4.
#
# CELL 5:  Butterworth's impulse response is a smooth symmetric pulse
#          (zero-phase due to filtfilt).  The boxcar is exactly rectangular
#          — every input sample is "spread out" uniformly into a 50 ms
#          rectangle, with sharp on/off edges that produce the sidelobes
#          you saw in CELL 2.
#
# CELL 6:  step response — phoneme onsets are S-shaped under the
#          Butterworth (smooth rise) and linear-ramp shaped under the
#          boxcar.  Both reach their final value in roughly the same time,
#          but the Butterworth has no "edges" that the classifier might
#          confuse with the actual transition.
#
# CELL 7:  Three views of "what changes with order N":
#            A. passband stays flat longer at higher N
#            B. dB rolloff slope = 6·N dB/oct (12·N with filtfilt)
#            C. transition width (−1 dB → −40 dB) shrinks with N
#
# CELL 8:  The price you pay for sharper frequency response: wider time-
#          domain impulse response.  Higher N = the filter remembers each
#          input sample for longer.  This is the time–frequency uncertainty
#          principle — you cannot have both a sharp knee AND a tight
#          impulse response.  N=4 is the chosen compromise.
#
# CELL 9:  The trade-off in numbers — rolloff slope, transition width,
#          impulse-response duration as a function of N.  N=4 gives 48 dB/oct
#          effective rolloff and a ~100 ms impulse response width, which
#          matches phoneme duration cleanly.
