# ============================================================
# explained_v1_pipeline.py — visual walkthrough of the v1 pipeline
#
# Each `# %%` block is a Jupyter cell.  Run top-to-bottom on a fresh kernel.
# Each cell produces at least one figure.  The story:
#
#   raw EEG → bandpass + envelope → standardize → causal TCN → 128-d frames
#   → per-phoneme average → LDA → log-probs → smooth → Viterbi → phonemes
#
# We use P22 throughout for consistency and a single train sentence as the
# running example.  Cell 4 either loads the trained v1 encoder from disk OR
# does a brief in-line SSL pretrain on one patient (~5 min) so later cells
# have an encoder to inspect.
# ============================================================
