...
class AcousticChangeDetector(DebugMixin):
    ...
    def prepare_phoneme_training_data(self, enhanced_batch, **kwargs):
            ...
            try:
                ...
                if self.decoder is not None:
                    ...
                    if feature_extraction_method == 'high_gamma':
                        ...
                        feat = extractHG(eeg, 1024, debug=self.DEBUG_MODE)  # Using imported function
                    ...
                    elif self.decoder is not None and hasattr(self.decoder, 'custom_feature_extraction'):
                        # Use the decoder's custom_feature_extraction method for any method
                        self.debug(f"Using decoder's custom_feature_extraction with method: {feature_extraction_method}")
        ...
        # Apply PCA if requested - SINGLE PCA SECTION
        if pca_components is not None and pca_components > 0 and features:
            # Check if decoder has a PCA model we can reuse
            pca_model = None
            if self.decoder is not None and hasattr(self.decoder, 'get_pca_model'):
            ...
            if pca_model is not None:
                ...
                # Get expected feature dimension from the PCA model
                expected_features = pca_model.n_features_in_
                self.debug(f"PCA model expects {expected_features} features")
                
                # Apply the PCA model with padding if needed
                for i in range(len(features)):
                    if features[i].shape[1] == expected_features:
                        # Direct transform if dimensions match
                        features[i] = pca_model.transform(features[i])
                    elif features[i].shape[1] < expected_features:
                        # Apply zero padding to match expected dimensions
                        self.debug(f"Padding features from {features[i].shape[1]} to {expected_features}")
                        padded = np.zeros((features[i].shape[0], expected_features))
                        padded[:, :features[i].shape[1]] = features[i]
                        features[i] = pca_model.transform(padded)
                    else:
                        # Truncate if features exceed expected dimensions
                        self.debug(f"Truncating features from {features[i].shape[1]} to {expected_features}")
                        features[i] = pca_model.transform(features[i][:, :expected_features])
            else:
                # Create new PCA model
                self.debug("Creating new PCA model")
                
                # Find the maximum feature dimension across all segments
                max_features = max(f.shape[1] for f in features) if features else 0
                self.debug(f"Maximum feature dimension: {max_features}")
                
                if max_features > 0:
                    # Ensure pca_components doesn't exceed available features
                    n_components = min(pca_components, max_features)
                    
                    # Create PCA with adjusted components
                    pca = PCA(n_components=n_components)
                    
                    # Create a list to store padded features for stacking
                    padded_features = []
                    
                    # First pass: zero-pad each feature array to have the same number of columns
                    for i, feat in enumerate(features):
                        if feat.shape[1] < max_features:
                            # Create zero-padded array
                            padded = np.zeros((feat.shape[0], max_features))
                            padded[:, :feat.shape[1]] = feat  # Copy original data
                            padded_features.append(padded)
                        else:
                            padded_features.append(feat)
                    
                    # Now stack the padded features
                    try:
                        all_features = np.vstack(padded_features)
                        pca.fit(all_features)
                        
                        # Transform features using the fitted PCA (apply padding first)
                        for i in range(len(features)):
                            if features[i].shape[1] < max_features:
                                # Create zero-padded array for transformation
                                padded = np.zeros((features[i].shape[0], max_features))
                                padded[:, :features[i].shape[1]] = features[i]
                                features[i] = pca.transform(padded)
                            else:
                        ...
                        if self.decoder is not None and hasattr(self.decoder, 'pca_models'):
                            ...
                            self.debug(f"Stored PCA model with {n_components} components in decoder")
                            
                    except ValueError as e:
                        self.log(f"Error during PCA: {e}")
                        self.debug("Dimensions of features:")
                        for i, feat in enumerate(features):
                            self.debug(f"Feature {i}: shape={feat.shape}")
                        # Continue without PCA if stacking fails
                        self.log("Continuing without PCA due to dimension mismatch")
