...
class UnifiedPhonemePipeline(DebugMixin):
    ...
    def __init__(self, path_bids, path_output, path_results, feature_extraction_method='high_gamma', 
             unknown_keep_ratio=0.1, pca_components=50, debug_mode=False, **kwargs):      
        ...
        self.step_outputs = {}
            ...
            # Update the config passed to CustomBrainAudioDecoder
        self.config = {
        ...
        self.log(f"Pipeline initialized with feature_extraction_method={self.feature_extraction_method}")

        self.log(f"Pipeline initialized with PCA components={pca_components}")
    ...
    def step2_stratify_participants(self, channel_correlation_threshold=0.1):
        ...
        self.debug("Stratifying participants...")
        ...
        self.log("Step 2: Participants stratified")
        ...
        # Save metadata and smaller components first
            ...
            with open(filepath, 'wb') as f:
                pickle.dump({
                    ...
                    # Don't include large objects here
                }, f)
    ...
    def step7_filter_unknowns(self, unknown_keep_ratio=None):
        ...
        # Ensure there's a group for unknown phonemes
        if '?' not in temp_model.phoneme_to_group:
            temp_model.phoneme_to_group['?'] = 'unknown'
