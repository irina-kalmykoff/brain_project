# Converted from ssl_lda_frames_consistent_test_split.ipynb
...
ENCODER_NAME = lambda pid: os.path.join(MODEL_DIR, f'{pid}_ssl_encoder_crfsplit.pt')
os.makedirs(MODEL_DIR, exist_ok=True)

DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
print(f"DEVICE = {DEVICE}")

# Build pipeline, accomodate CRF test split
# Build datasets on the same sentences the CRF was tested on, for consistency across
# ablations. Replaces the TEST_OFFSET::6 convention from ssl_lda_frames_clean.py;
# build_sentence_dataset (below) reads CRF_TEST to assign train/test.
import pickle
_crf_export = pickle.load(open('results/crf_export.pkl', 'rb'))
CRF_TEST = {pid: set(np.asarray(_crf_export[pid]['true_sentence_ids']).astype(int).tolist())
            for pid in _crf_export}
print(f"CRF test split loaded for {len(CRF_TEST)} patients")
...
def build_sentence_dataset(pid, bands=BANDS):
    ...
    test_sent_ids = CRF_TEST.get(pid, set())
...
# RUN THIS IN THE SSL consistent-split NOTEBOOK (not the report notebook)
import pickle, os
os.makedirs('results', exist_ok=True)
pickle.dump({'split': 'crf',
             'acc_frame': ssl_acc, 'ce_frame': ssl_ce,
             'acc_pho': ssl_acc_pp, 'ce_pho': ssl_ce_pp},
            open('results/ssl_shift_perm_crfsplit.pkl', 'wb'))
print('saved (from SSL kernel):', {k: type(v).__name__ for k,v in
      {'acc_pho': ssl_acc_pp}.items()})

# cohort inference + NW metrics + save — CONSISTENT (CRF) split
import warnings
warnings.filterwarnings('ignore', message='Only one sample available')
ssl_results = {}
...
if ssl_results:
    ...
    with open('results/ssl_only/hg_only_cohort_crfsplit.pkl', 'wb') as f:   # (1) renamed
        pickle.dump(ssl_results, f)
    pickle.dump(ssl_results, open('results/ssl_results_crfsplit.pkl', 'wb')) # (2) for the report
    summary = {'spec': 'HG_only_crfsplit', 'split': 'crf',                   # (3) tag the split
               'bands': BANDS, 'SSL_EPOCHS': SSL_EPOCHS,
    ...
    with open('results/ssl_only/hg_only_summary_crfsplit.json', 'w') as f:   # (1) renamed
        json.dump(summary, f, indent=2)
    print("saved -> hg_only_cohort_crfsplit.pkl + hg_only_summary_crfsplit.json + ssl_results_crfsplit.pkl")
