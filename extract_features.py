...
def downsampleLabels(labels, sr, windowLength=0.05, frameshift=0.01):
    ...
    #return newLabels
        ...
        # Convert to bytes if needed (for compatibility)
        if isinstance(most_common_value, str):
            newLabels[w] = most_common_value.encode("ascii", errors="ignore")
        else:
            newLabels[w] = most_common_value
            
    # Convert byte strings back to regular strings when returning
