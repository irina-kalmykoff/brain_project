...
def crf_stacking_order(scaler, n_channels):
    """Recover the stacking order the per-patient CRF was trained with, from the
    fitted scaler's feature dimension and the channel count, so inference feature
    extraction always matches training without depending on a hand-set
    RUN_CONFIG['stacking_order']. Falls back to RUN_CONFIG if the dimension is not
    a clean (2*order+1) multiple of n_channels."""
    feat_dim = getattr(scaler, 'n_features_in_', None)
    if feat_dim and n_channels and feat_dim % n_channels == 0:
        frames = feat_dim // n_channels
        if frames % 2 == 1:
            return (frames - 1) // 2
    return RUN_CONFIG['stacking_order']
...
def evaluate_detector_path(pipeline, classifiers,
    ...
    # stk_order is derived per patient from the CRF scaler inside the loop
    # (crf_stacking_order), so it always matches the training features.
    ...
    for item in detector_test_ds:
        ...
        stk_order = crf_stacking_order(classifiers[pid]['scaler'],
                                       sentence_eeg.shape[1])
...
def run_with_mfa_oracle_boundaries(pipeline, run_config=None):
    """Oracle MFA-CRF for the time-aligned visualisation.

    Identical to `run_with_ieeg_boundaries` except the segmentation comes from
    the MFA gold boundaries instead of the iEEG detector. The CRF therefore
    classifies exactly one phoneme per gold segment, so the prediction is 1:1
    with gold and shares its time stamps (true_segments == pred_segments). Use
    this to visualise the headline MFA-CRF: every gold slot gets a predicted
    phoneme, and the matched runs are driven largely by the class prior and the
    CRF's phonotactic transitions rather than by neural content.

    For illustration only: it applies the CRF one sequence per sentence (as the
    brain-only path does) and drops the rare degenerate segment where feature
    extraction fails; it is not a re-derivation of the headline PER.
    """
    classifiers, _v6_model, detector_test_ds = train_brain_only_models(pipeline)
    ...
    acc = defaultdict(lambda: {'true': [], 'pred': [], 'tsid': [], 'psid': [],
                               'tseg': [], 'pseg': []})
    ...
    for item in detector_test_ds:
        ...
        crf    = classifiers[pid]['crf']
        valid  = classifiers[pid]['valid_classes']
        stk_order = crf_stacking_order(scaler, sentence_eeg.shape[1])

        # Oracle segmentation: one segment per MFA gold phoneme, filtered to the
        # training class set (same filter the gold labels / true_segs use).
        mfa_phones = [p for p in load_mfa_alignments(pid).get(sent_idx, [])
                      if p['phone'] in valid]
        feats, segs, gold = [], [], []
        for p in mfa_phones:
            f = extract_phoneme_feature_pipeline_native(
                sentence_eeg, p['start_s'], p['end_s'],
                eeg_sr, win_len, frameshift, stk_order)
            if f is None:
                continue
            feats.append(f)
            segs.append((p['start_s'], p['end_s']))
            gold.append(p['phone'])
        if not feats:
            continue

        X = pca.transform(scaler.transform(np.asarray(feats)))
        y_pred = list(crf.predict(_features_to_dicts_one_seq(X))[0])

        d = acc[pid]
        d['true'].extend(gold)
        d['pred'].extend(y_pred)
        d['tsid'].extend([sent_idx] * len(gold))
        d['psid'].extend([sent_idx] * len(y_pred))
        d['tseg'].extend(segs)
        d['pseg'].extend(segs)               # oracle: identical boundaries -> 1:1

    pipeline.patient_results = {}
    for pid, d in sorted(acc.items()):
        if not d['true']:
            continue
        ed = edit_distance(d['true'], d['pred'])
        common = min(len(d['true']), len(d['pred']))
        accuracy = (sum(t == p for t, p in zip(d['true'][:common], d['pred'][:common]))
                    / max(len(d['true']), 1))
        pipeline.patient_results[pid] = {
            'true_labels':       d['true'],
            'predictions':       d['pred'],
            'true_sentence_ids': d['tsid'],
            'pred_sentence_ids': d['psid'],
            'true_segments':     d['tseg'],
            'pred_segments':     d['pseg'],
            'accuracy':          accuracy,
            'edit_distance':     ed,
            'per':               ed / max(len(d['true']), 1),
            'n_test':            len(d['true']),
    ...
    return ('mfa_oracle_path', {'k_source': 'mfa_gold'},
            pipeline.patient_results)
...
def _time_merge_columns(true_segs, pred_segs, tol_s=0.04):
    """Two-pointer merge by start_s into shared columns.

    Returns list of (t_idx_or_None, p_idx_or_None). true[i] and pred[j] share a
    column only when their start times are within tol_s AND they are mutual
    nearest neighbours (neither has a strictly closer partner among the other
    side's upcoming events). Otherwise the earlier event is emitted alone, with a
    blank on the other side, so that same-time phonemes line up vertically and an
    extra or skipped phoneme on one side does not shift the rest of the row.
    """
    def t_of(seg):
    ...
    while i < n_t or j < n_p:
        ...
        # missing time(s): fall back to positional pairing / emit what we have
        if t is None and p is None:
            ...
            continue
        if p is None:
            cols.append((i, None)); i += 1; continue
        if t is None:
            cols.append((None, j)); j += 1; continue

        if abs(t - p) <= tol_s:
            # within tolerance: merge only if neither side has a closer partner
            t_next = t_of(true_segs[i + 1]) if i + 1 < n_t else None
            p_next = t_of(pred_segs[j + 1]) if j + 1 < n_p else None
            t_prefers_next = p_next is not None and abs(t - p_next) < abs(t - p)
            p_prefers_next = t_next is not None and abs(t_next - p) < abs(t - p)
            if not t_prefers_next and not p_prefers_next:
                cols.append((i, j)); i += 1; j += 1          # mutual nearest -> merge
            elif p <= t:
                cols.append((None, j)); j += 1               # release earlier pred
            else:
                cols.append((i, None)); i += 1               # release earlier true
        elif t < p:
            cols.append((i, None)); i += 1                   # true earlier, alone
        else:
            cols.append((None, j)); j += 1                   # pred earlier, alone
    return cols
    ...
    def render_sentence_block(t_lo, t_hi, p_lo, p_hi, sid=None):
        ...
        if sid is not None:
            ...
            chunks.append(
                ...
                f'sentence {(sentence_ordinal or {}).get(sid, sid)} '
                f'(stim {sid})  (true={len_t}, pred={len_p}){mode}'
                f'{text_html}'
                f'</div>'
            )
        for chunk_start in range(0, L, max_per_line):
...
def show_matched_sequences_with_times(pipeline, pid,
    ...
    # Build {sid: text} and {sid: ordinal-among-real-sentences} for this patient.
    # sentence_list interleaves real sentences with empty inter-stimulus slots, so
    # the raw index (e.g. 138) is far higher than the real-sentence count. The
    # ordinal counts only non-empty entries, so raw sid 138 -> "sentence 69".
    sentence_texts = {}
    sentence_ordinal = {}
    try:
