...
class PhonemeDecoderModel(DebugMixin):
    ...
    def prepare_data(self, features, labels, validation_split=0.2, test_split=0.1, max_length=None):
        ...
        if self.decoder is not None:
            ...
            # Get PCA components from config
            pca_components = config.get('pca_components', None)
            use_pca = pca_components is not None
            
            self.debug(f"Using decoder's config with PCA components={pca_components}")
        else:
            # Use provided kwargs if no decoder
            use_pca = kwargs.get('use_pca', False)
            pca_components = kwargs.get('pca_components', 50) if use_pca else None
            
            self.debug(f"No decoder available, using provided kwargs: PCA components={pca_components}")
