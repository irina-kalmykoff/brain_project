...
class PhonemeFeatureVisualizer:
    ...
    def visualize_phoneme_features(self, phoneme_segments, phoneme_name, eeg_sr=1024, 
        ...
        """
        Generate multiple visualizations for a specific phoneme.
        
        Parameters:
        -----------
        phoneme_segments : list
            List of iEEG segments for the phoneme
        phoneme_name : str
            Name of the phoneme
        eeg_sr : int
        ...
        band : tuple
            Frequency band to analyze (for high gamma, etc.)
        """
        # Create output directory for this phoneme
        phoneme_dir = os.path.join(self.output_dir, f"phoneme_{phoneme_name}")
        os.makedirs(phoneme_dir, exist_ok=True)
        
        # Apply filtering for the specified band
        filtered_segments = []
        ...
        fig.suptitle(f"Phoneme '{phoneme_name}' - {method} ({band[0]}-{band[1]} Hz)", fontsize=16)
        
        # 1. ERP (Event-Related Potential) - Time Series
        self._plot_erp(axs[0, 0], filtered_segments, eeg_sr)
        axs[0, 0].set_title("ERP Time Series")
        
        # 2. Amplitude Time Series (using Hilbert transform)
        self._plot_amplitude(axs[0, 1], filtered_segments, eeg_sr)
        axs[0, 1].set_title("Amplitude Time Series")
        
        # 3. Phase Angle Time Series (using Hilbert transform)
        self._plot_phase(axs[1, 0], filtered_segments, eeg_sr)
        axs[1, 0].set_title("Phase Angle Time Series")
        
        # 4. Frequency Domain (Power Spectrum)
        self._plot_spectrum(axs[1, 1], phoneme_segments, eeg_sr)
        axs[1, 1].set_title("Power Spectrum")
        
        # 5. Static Power Spectrum (averaged across time)
        self._plot_static_spectrum(axs[2, 0], phoneme_segments, eeg_sr, band)
        axs[2, 0].set_title("Static Power Spectrum")
        
        # 6. Time-Frequency Plot (for high gamma)
        self._plot_time_frequency(axs[2, 1], phoneme_segments[0], eeg_sr)
        axs[2, 1].set_title("Time-Frequency Plot (Example)")
        
        plt.tight_layout(rect=[0, 0, 1, 0.96])  # Make room for suptitle
        plt.savefig(os.path.join(phoneme_dir, f"{method}_summary.png"), dpi=300)
        plt.close()
