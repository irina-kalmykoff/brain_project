# Converted from parse_features_of_30_patients_whisper_mfa_whisper_test.ipynb
...
from run_pipeline import compare_classifiers, run_experiment

def run_rf_with_features(pipeline, features_train, features_test, label):
    """Run Random Forest with custom features, no PCA."""
    ...
    try:
        ...
        # Use run_experiment with classifier_type='random_forest'
        cfg = dict(run_config)
        cfg['classifier_type'] = 'random_forest'
        name, params, results = run_experiment(
            pipeline,
            order=cfg.get('markov_order', 1),
            class_weight=cfg.get('class_weight', 'balanced'),
            classifier_type='random_forest',
            use_viterbi=False,
            stacking_order=cfg.get('stacking_order'),
            stacking_step_size=cfg.get('stacking_step_size'),
            scaler_type='standard',
            min_class_samples=cfg.get('min_class_samples', 5),
        )
    finally:
...
print("\nRF on iEEG…");    rf_ieeg = run_rf_with_features(pipeline, ieeg_train_z, ieeg_test_z, 'rf-ieeg')
print("RF on Whisper…");   rf_wsp  = run_rf_with_features(pipeline, whisp_train_z, whisp_test_z, 'rf-wsp')
print("RF on concat…");    rf_both = run_rf_with_features(pipeline, concat_train_z, concat_test_z, 'rf-concat')

# Compare
pids_sorted = sorted(rf_ieeg.keys())
print(f"\n  {'pid':<6} {'iEEG':>8} {'Whisper':>10} {'concat':>10}   Δ vs Whisper")
print("  " + "-" * 56)
for pid in pids_sorted:
    print(f"  {pid:<6} {rf_ieeg[pid]['accuracy']:>7.1%} "
          f"{rf_wsp[pid]['accuracy']:>9.1%} {rf_both[pid]['accuracy']:>9.1%}   "
          f"{rf_both[pid]['accuracy'] - rf_wsp[pid]['accuracy']:>+13.1%}")
mean_ieeg = np.mean([r['accuracy'] for r in rf_ieeg.values()])
mean_wsp  = np.mean([r['accuracy'] for r in rf_wsp.values()])
mean_both = np.mean([r['accuracy'] for r in rf_both.values()])
print("  " + "-" * 56)
print(f"  {'mean':<6} {mean_ieeg:>7.1%} {mean_wsp:>9.1%} {mean_both:>9.1%}   "
      f"{mean_both - mean_wsp:>+13.1%}")

# ═══════════════════════════════════════════════════════════════════════════════
# CELL 4 (REPLACEMENT v3) — Sequence-aligned phoneme-level Whisper features
# ═══════════════════════════════════════════════════════════════════════════════
# inst_ids enumerate train phonemes (or test phonemes) in chronological order
# but NOT global MFA order. So we sequence-align: walk pipeline.train phonemes
# alongside MFA TextGrid phonemes, advancing the TextGrid pointer past
# unmatched (test) phonemes when labels disagree.
...
def encode_full_audio_whisper(audio, sr_audio):
    target_sr = 16000
    if sr_audio != target_sr:
        from scipy.signal import resample_poly
        g = np.gcd(int(sr_audio), target_sr)
        audio = resample_poly(audio, target_sr // g, sr_audio // g).astype(np.float32)
    else:
    ...
    for s in range(0, len(audio), chunk_samples):
        seg = audio[s:s + chunk_samples]
        if len(seg) < chunk_samples:
            seg = np.pad(seg, (0, chunk_samples - len(seg)))
        mel = whisper.log_mel_spectrogram(torch.from_numpy(seg)).to(DEVICE)
        with torch.no_grad():
            emb = whisper_model.encoder(mel.unsqueeze(0))
        chunks.append(emb.squeeze(0).cpu().numpy().astype(np.float32))
    return np.concatenate(chunks, axis=0)


def whisper_feature_for_window(encoder_emb, start_s, end_s):
    f_start = max(0, int(start_s * 50))
    f_end   = min(encoder_emb.shape[0], int(end_s * 50))
    if f_end <= f_start:
        f_end = f_start + 1
    return encoder_emb[f_start:f_end].mean(axis=0)


def build_global_phoneme_timings(pipeline, mfa_dir='mfa_output'):
    """Per patient: walk MFA TextGrids in sentence_idx order, build a flat list
    of (sent_idx, phone_text, abs_start_s, abs_end_s)."""
    import tgt
    timings = {}
    ...
    for pid in sorted(set(pipeline.train['phoneme_participant_ids'])):
        ...
        if not word_data:
            continue
        sent_list = word_data['sentence_list']
        ...
        for sent_idx, sent_info in enumerate(sent_list):
            if not isinstance(sent_info, dict):
                continue
            sent_offset_s = sent_info['stim_start_idx'] / eeg_sr
            ...
            for ann in phone_tier.annotations:
                ...
                if text and text not in ('sp', 'sil', 'spn'):
                    flat.append((sent_idx, text,
                                 sent_offset_s + ann.start_time,
                                 sent_offset_s + ann.end_time))
        timings[pid] = flat
        print(f"  {pid}: {n_loaded} TextGrids → {len(flat)} phonemes")
    return timings


def align_pipeline_to_mfa(pipeline, timings,
                           max_skip_per_phoneme=200):
    """Sequence-align pipeline.train/test phonemes against MFA list per patient.
    
    For each patient, sort phonemes by inst_id (chronological order), then walk
    MFA list in parallel, advancing MFA pointer past mismatches (=test phonemes
    when processing train, or vice versa).
    """
    out = {'train': [None] * len(pipeline.train['features']),
           'test':  [None] * len(pipeline.test['features'])}
    encoder_cache = {}
    stats = defaultdict(int)

    for ds_name in ['train', 'test']:
        data = getattr(pipeline, ds_name)
        pids_arr  = data['phoneme_participant_ids']
        ids_arr   = data['phoneme_instance_ids']
        labels_arr = data['phoneme_labels']

        # Per patient: sort phoneme indices by inst_id (chronological)
        for pid in sorted(set(pids_arr)):
            patient_indices = [i for i, p in enumerate(pids_arr) if p == pid]
            patient_indices.sort(key=lambda i: int(ids_arr[i].split('_')[-1]))

            mfa_list = timings.get(pid, [])
            if not mfa_list:
                for i in patient_indices:
                    out[ds_name][i] = np.zeros(WHISPER_DIM, dtype=np.float32)
                    stats['no_mfa'] += 1
                continue

            # Load encoder once per patient
            if pid not in encoder_cache:
                cf = cache_path(pid)
                if os.path.exists(cf):
                    with open(cf, 'rb') as f:
                        encoder_cache[pid] = pickle.load(f)
                else:
                    raw = pipeline.dutch30_extractor.load_patient_raw_data(pid)
                    encoder_cache[pid] = encode_full_audio_whisper(
                        raw['audio'], pipeline.config.audio_sr)
                    with open(cf, 'wb') as f:
                        pickle.dump(encoder_cache[pid], f)
            encoder_emb = encoder_cache[pid]

            # Walk MFA list and pipeline phonemes together
            mfa_ptr = 0
            for i in patient_indices:
                target_label = labels_arr[i]

                # Advance MFA pointer up to max_skip_per_phoneme to find a label match
                skip = 0
                while (mfa_ptr < len(mfa_list)
                       and mfa_list[mfa_ptr][1] != target_label
                       and skip < max_skip_per_phoneme):
                    mfa_ptr += 1
                    skip += 1

                if mfa_ptr >= len(mfa_list):
                    out[ds_name][i] = np.zeros(WHISPER_DIM, dtype=np.float32)
                    stats['mfa_exhausted'] += 1
                    continue

                if mfa_list[mfa_ptr][1] == target_label:
                    sent_idx, ph_text, start_s, end_s = mfa_list[mfa_ptr]
                    out[ds_name][i] = whisper_feature_for_window(encoder_emb, start_s, end_s)
                    mfa_ptr += 1
                    stats['matched'] += 1
                    if skip > 0:
                        stats[f'skipped_{ds_name}'] += skip
                else:
                    # Couldn't find within max_skip — give up on this phoneme
                    out[ds_name][i] = np.zeros(WHISPER_DIM, dtype=np.float32)
                    stats['no_match_in_window'] += 1

    print(f"\n  Alignment stats:")
    for k, v in sorted(stats.items()):
        print(f"    {k:<22} {v:>8}")

    n_zero_train = sum(1 for f in out['train'] if not np.any(f))
    n_zero_test  = sum(1 for f in out['test']  if not np.any(f))
    print(f"\n  Zero-vector phonemes:")
    print(f"    train {n_zero_train}/{len(out['train'])} "
          f"({n_zero_train/len(out['train'])*100:.1f}%)")
    print(f"    test  {n_zero_test}/{len(out['test'])}  "
          f"({n_zero_test/len(out['test'])*100:.1f}%)")
    return out


# ─── Run it ───────────────────────────────────────────────────────────────────
print("Building global phoneme timing maps from MFA TextGrids…")
timings = build_global_phoneme_timings(pipeline)

print("\nSequence-aligning pipeline phonemes to MFA timings…")
whisper_feats = align_pipeline_to_mfa(pipeline, timings)

from collections import defaultdict


def zscore_features_per_patient(features_list, pids_list):
    """Per-feature z-score computed independently per patient.
    
    Different patients may have different feature dimensions (different
    channel counts after exclusions). Within a patient, shapes are uniform.
    """
    pid_indices = defaultdict(list)
    for i, p in enumerate(pids_list):
        pid_indices[p].append(i)

    out = [None] * len(features_list)
    for pid, indices in pid_indices.items():
        feats = [features_list[i] for i in indices]
        try:
            arr = np.stack(feats).astype(np.float32)
        except ValueError:
            shapes = sorted(set(f.shape for f in feats))[:3]
            print(f"  WARN {pid}: mixed shapes within patient {shapes}, skipping z-score")
            for i in indices:
                out[i] = features_list[i].astype(np.float32)
            continue
        mean = arr.mean(axis=0)
        std  = arr.std(axis=0) + 1e-6
        for k, i in enumerate(indices):
            out[i] = (arr[k] - mean) / std
    return out


print("Z-scoring each modality (per patient)…")
ieeg_train_z  = zscore_features_per_patient(
    pipeline.train['features'],   pipeline.train['phoneme_participant_ids'])
ieeg_test_z   = zscore_features_per_patient(
    pipeline.test['features'],    pipeline.test['phoneme_participant_ids'])
whisp_train_z = zscore_features_per_patient(
    whisper_feats['train'],       pipeline.train['phoneme_participant_ids'])
whisp_test_z  = zscore_features_per_patient(
    whisper_feats['test'],        pipeline.test['phoneme_participant_ids'])

# Spot-check
print(f"  iEEG    train P21: dim={ieeg_train_z[0].shape}, "
      f"sample mean={ieeg_train_z[0].mean():.4f}, std={ieeg_train_z[0].std():.4f}")
print(f"  Whisper train P21: dim={whisp_train_z[0].shape}, "
      f"sample mean={whisp_train_z[0].mean():.4f}, std={whisp_train_z[0].std():.4f}")

# Concat (still varies per patient because iEEG dim varies)
concat_train_z = [np.concatenate([a, b]) for a, b in zip(ieeg_train_z, whisp_train_z)]
concat_test_z  = [np.concatenate([a, b]) for a, b in zip(ieeg_test_z,  whisp_test_z)]
...
print("\n" + "═"*60)
print(" EXP 1: iEEG only (z-scored)")
print("═"*60)
results_ieeg = run_with_features(pipeline, ieeg_train_z, ieeg_test_z, 'iEEG-z')
accs_ieeg, lifts_ieeg = summarize(results_ieeg, 'iEEG-z')

print("\n" + "═"*60)
print(" EXP 2: Whisper only (z-scored)")
print("═"*60)
results_wsp = run_with_features(pipeline, whisp_train_z, whisp_test_z, 'Whisper-z')
accs_wsp, lifts_wsp = summarize(results_wsp, 'Whisper-z')

print("\n" + "═"*60)
print(" EXP 3: iEEG + Whisper (z-scored, concat)")
print("═"*60)
results_both = run_with_features(pipeline, concat_train_z, concat_test_z, 'concat-z')
accs_both, lifts_both = summarize(results_both, 'iEEG+Whisper-z')

# Comparison table
pids_sorted = sorted(results_ieeg.keys())
...
for pid in pids_sorted:
    ...
    print(f"  {pid:<6} {a_i:>7.1%} {a_w:>9.1%} {a_b:>9.1%}   {a_b - a_w:>+13.1%}")
...
# ═══════════════════════════════════════════════════════════════════════════════
# 1. Rebuild pipeline up to (but NOT including) step5b stacking
# ═══════════════════════════════════════════════════════════════════════════════
# CAR is best applied on 2D features (n_frames, n_channels) before stacking.
# We can't use run_path_b directly because it does step5b internally.
...
f = pipeline.train['features'][0]
print(f"\nfeature shape: {f.shape}   ndim: {f.ndim}")
assert f.ndim == 2, "CAR requires 2D features (n_frames, n_channels) — pipeline state wrong"


# ═══════════════════════════════════════════════════════════════════════════════
# 2. Apply Common Average Reference
# ═══════════════════════════════════════════════════════════════════════════════
# For each phoneme's 2D feature matrix (n_frames, n_channels):
#   subtract per-frame mean across channels
#
# This removes signals shared across all channels (line interference, motion,
# common-mode drift) while preserving channel-specific spatial patterns.
...
for ds_name in ['train', 'test']:
    ...
    for i, f in enumerate(data['features']):
        # f shape: (n_frames, n_channels)
        car_correction = f.mean(axis=1, keepdims=True)   # (n_frames, 1)
        data['features'][i] = f - car_correction
        n_modified += 1
print(f"  CAR applied to {n_modified} phonemes")

# Quick sanity check — feature stats should change
sample = pipeline.train['features'][0]
print(f"  After CAR — sample mean per frame: "
      f"{sample.mean(axis=1)[0]:.6f} (should be ~0)")
print(f"  After CAR — overall std: {sample.std():.4f}")


# ═══════════════════════════════════════════════════════════════════════════════
# 3. Stacking + collapse + CRF
# ═══════════════════════════════════════════════════════════════════════════════
print("\nRunning step5b/c (stacking + collapse)…")
_run_step5abc(pipeline, run_config)

f = pipeline.train['features'][0]
print(f"  feature shape after stacking: {f.shape}")

# ═══════════════════════════════════════════════════════════════════════════════
# 4. Run CRF and compare
# ═══════════════════════════════════════════════════════════════════════════════
print("\nRunning CRF on CAR features…")
pipeline.patient_results = {}
crf_results_car = _run_crf_experiment(pipeline, run_config)

print("\n  pid    accuracy    lift")
print("  " + "-" * 28)
accs_car, lifts_car = [], []
for pid, r in crf_results_car.items():
    n_cl = len(set(r['true_labels']))
    chance = 1.0 / n_cl if n_cl > 0 else 0
    lift = r['accuracy'] / chance if chance > 0 else 0
    accs_car.append(r['accuracy']); lifts_car.append(lift)
...
print("  " + "-" * 28)
print(f"  mean   {np.mean(accs_car):>5.1%}    {np.mean(lifts_car):.2f}x")
